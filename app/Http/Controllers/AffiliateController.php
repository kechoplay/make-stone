...
class AffiliateController extends Controller
{
    ...
    function restore(Request $request) {
        try {
            $id = $request->get('id');
            $restore = Affiliate::onlyTrashed()->where('id',$id)->restore(); //khoi phuc
            if ($restore) {
                return response()->json(['status' => 'success', 'message' => 'Khôi phục đường dẫn thành công'], 200);
            } else {
                return response()->json(['status' => 'error', 'message' => 'Không tồn tại mã đường dẫn ' . $id . ' trong thùng rác'], 404);
            }
        } catch (QueryException $e) {
            return response()->json([
                'status' => 'error',
